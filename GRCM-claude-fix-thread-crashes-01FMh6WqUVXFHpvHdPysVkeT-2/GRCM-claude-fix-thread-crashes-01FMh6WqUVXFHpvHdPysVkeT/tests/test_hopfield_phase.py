"""
Validation Suite for Phase-Only Hopfield Lattice

Tests all 5 metrics with phase-based pseudo-inverse:
1. Self-healing > 70%
...
This fixes the scalar/vector mismatch by working entirely in phase space.
"""
...
from grcm.echozero.identity.torsion_lattice.hopfield_phase import HopfieldTorsionLatticePhase
...
def test_self_healing():
    ...
    lattice = HopfieldTorsionLatticePhase(size=5)
    ...
    print(f"✓ Learning {len(patterns)} patterns via phase-only pseudo-inverse...")
    ...
    noise_phases = np.random.uniform(-np.pi, np.pi, size=lattice.phases.shape)
    lattice.phases += noise_phases
    lattice.phases = lattice.phases % (2 * np.pi)
...
def test_capacity():
    ...
    lattice = HopfieldTorsionLatticePhase(size=5, max_patterns=20)
    ...
    print(f"✓ Learning {num_patterns} patterns via phase-only pseudo-inverse...")
...
def main():
    """Run all phase-only pseudo-inverse validation tests"""
    print("╔" + "="*68 + "╗")
    print("║" + " "*5 + "PHASE-ONLY HOPFIELD LATTICE - VALIDATION SUITE" + " "*16 + "║")
    print("║" + " "*8 + "Pseudo-Inverse on Scalar Phases (Fixed Implementation)" + " "*6 + "║")
    print("╚" + "="*68 + "╝")
    ...
    if tests_passed == total_tests:
        print("✅ ALL TESTS PASSED - Phase-only pseudo-inverse validated!")
        print("\nThe lattice now:")
        ...
        print("  Phase-only v2:   Current results above")
