"""
Validation Suite for Complex-Valued Hopfield Lattice

Tests all 5 metrics with complex pseudo-inverse:
1. Self-healing > 70%
...
This is the CORRECT implementation using complex numbers to handle
2D vector topology properly.
"""
...
from grcm.echozero.identity.torsion_lattice.hopfield_complex import HopfieldTorsionLatticeComplex
...
def test_self_healing():
    ...
    lattice = HopfieldTorsionLatticeComplex(size=5)

    # Learn 3 patterns via complex pseudo-inverse
    patterns = [
        np.array([1.0, 0.0]),
        np.array([0.0, 1.0]),
        np.array([0.7, 0.7]),
    ...
    print(f"✓ Learning {len(patterns)} patterns via complex pseudo-inverse...")
    ...
    # Corrupt with massive noise (random complex phases)
    print("✓ Injecting massive noise...")
    noise_phases = np.random.uniform(0, 2*np.pi, size=lattice.state.shape)
    noise = np.exp(1j * noise_phases)
    lattice.state = (lattice.state + noise * 2.0)
    # Renormalize
    lattice.state = lattice.state / (np.abs(lattice.state) + 1e-8)
...
def test_capacity():
    ...
    lattice = HopfieldTorsionLatticeComplex(size=5, max_patterns=20)
    ...
    print(f"✓ Learning {num_patterns} patterns via complex pseudo-inverse...")
...
def main():
    """Run all complex-valued pseudo-inverse validation tests"""
    print("╔" + "="*68 + "╗")
    print("║" + " "*3 + "COMPLEX-VALUED HOPFIELD LATTICE - VALIDATION SUITE" + " "*14 + "║")
    print("║" + " "*6 + "Pseudo-Inverse in Complex Domain (CORRECT Implementation)" + " "*4 + "║")
    print("╚" + "="*68 + "╝")
    ...
    if tests_passed == total_tests:
        print("✅ ALL TESTS PASSED - Complex-valued pseudo-inverse validated!")
        print("\nThe lattice now:")
        ...
        print("  Pseudo-inv v1:     1/5 passing (scalar/vector mismatch)")
        print("  Phase-only v2:     1/5 passing (circular variable issue)")
        print(f"  Complex v3:        {tests_passed}/5 passing (current)")
