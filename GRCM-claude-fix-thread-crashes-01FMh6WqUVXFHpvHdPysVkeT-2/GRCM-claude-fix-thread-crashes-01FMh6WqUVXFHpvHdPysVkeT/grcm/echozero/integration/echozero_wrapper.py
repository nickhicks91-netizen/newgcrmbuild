...
class EchoZeroWrapper:
    ...
    def forward(self, x: np.ndarray) -> Dict[str, Any]:
        """
        The full EchoZero coherence pipeline.
        ...
        if self.cfg.enable_logging:
            self.logs["torsion_magnitude"].append(float(torsion_mag))
        ...
        if self.tachyon is not None:
            ...
            if tachyon_result['event_detected']:
                if self.cfg.enable_logging:
                    self.logs["tachyon_events"].append({
                        'step': self.step,
        ...
        # Step 5: Slow loop sync (non-blocking)
        if self.step - self.last_slow_update >= self.cfg.slow_loop_rate:
            self.slow_update()
            self.last_slow_update = self.step
    ...
    def slow_update(self):
        ...
        if self.cfg.enable_logging and self.tachyon is not None:
            self.logs["identity_snapshots"].append({
                'step': self.step,
    ...
    def export_state(self) -> Dict:
        """
        Export complete system state for visualization/logging.
        ...
        state = {
            'step': self.step,
            'torsion_magnitude': self.logs["torsion_magnitude"][-100:] if self.logs["torsion_magnitude"] else [],
            'recent_events': self.logs["tachyon_events"][-20:] if self.logs["tachyon_events"] else [],
        }
